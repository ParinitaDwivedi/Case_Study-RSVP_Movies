...
-- Dream Warrior Pictures and National Theatre Live production houses has produced the most number (3) of hit movies (average rating > 8)
...
ON m.id = g.movie_id
WHERE 
	m.country LIKE '%USA%' AND r.total_votes>1000 AND MONTH(date_published)=3 AND year=2017
GROUP BY g.genre
...
(
	...
	FROM genre AS g 
		...
		INNER JOIN names AS n ON rm.name_id=n.id
	WHERE 
		genre= 'drama' AND category= 'actress' AND avg_rating>8
	GROUP BY name
)
SELECT *, Rank() OVER(ORDER BY movie_count DESC) AS actress_rank FROM ranking LIMIT 3;
